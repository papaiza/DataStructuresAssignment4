...
public class Courses {
    ...
    public Edge<CourseRequisite> putRequisite(CourseNumber courseNum1, 
    		...
    		if(courseMap.checkKey(courseNum1) == false && courseMap.checkKey(courseNum2) == false){
    			throw new InvalidCourseNumberException("Invalid course code");
    		}
        	Edge<CourseRequisite> edge = getRequisite(courseNum1, courseNum2);
        	...
        	DFS(courseGraph, vertex2, known, forest);
        	
        	if(known.contains(vertex1)){
        		throw new CircularPreRequisiteException("Graph is cyclic");
        	}else{
